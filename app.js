/* ════════════════════════════════════════════════
   ...
   • No email / phone in registration
   • Rejoin via name + height (not phone)
   • Navbar tabs (no bottom nav)
   • Owner can delete members
   • PWA Service Worker for background alarms
   ════════════════════════════════════════════════ */
"use strict";
...
const App = {
  ...
  init() {
    ...
    // Inject mobile tab row into panels
  ...
  _injectMobileTabs() {
    // On mobile, inject a tab row at top of dashboard main since nav-tabs is hidden
  ...
  _subRoom() {
    ...
    db.ref('rooms/'+this.roomCode).on('value',snap=>{
      ...
      if(me){
        if(!this._entered){ this._entered=true; enterDashboard(data); }
        else renderAll(data);
      } else if(pend){
  ...
  _updatePWAUI(supported) {
    const badge = $('pwaBadge');
    const tip   = $('pwaAlarmTip');
    if(supported){
      if(badge) { badge.textContent='🔔 Background'; badge.style.display='inline'; }
      if(tip)   { tip.textContent='✅ Background alarms enabled — will ring even when browser is closed!'; tip.classList.add('show'); }
    } else {
      if(badge) { badge.textContent='⚠️ Browser only'; badge.style.display='inline'; }
      if(tip)   { tip.textContent='⚠️ Add this app to your Home Screen (Install PWA) for background alarm support.'; tip.classList.add('show'); tip.style.color='var(--accent4)'; tip.style.background='rgba(255,209,102,.08)'; tip.style.borderColor='rgba(255,209,102,.25)'; }
    }
  },
  ...
  _syncAlarmsToSW() {
    ...
    navigator.serviceWorker.ready.then(reg=>{
      if(reg.active) {
        reg.active.postMessage({ type:'SYNC_ALARMS', alarms:this.alarms, userId:this.userId });
      }
    }).catch(()=>{});
  },
};
...
const UI = {
  ...
  switchTab(tab, btn) {
    // Sync all tab buttons (both desktop nav and mobile row)
    document.querySelectorAll('.nav-tab,.tab-btn').forEach(b=>b.classList.remove('active'));
    document.querySelectorAll('.panel').forEach(p=>p.classList.remove('active'));
    // Activate matching buttons across both bars
...
// Listen for alarm message from service worker
if('serviceWorker' in navigator){
  navigator.serviceWorker.addEventListener('message', e=>{
    if(e.data?.type==='RING_ALARM'){
      Alarm.ring(e.data.time);
    }
  });
}
...
const Workouts = {
  async add() {
    ...
    const wid=genId(), entry={id:wid,type,duration,time,note,done:false,ts:Date.now()};
    try {
      await db.ref('rooms/'+App.roomCode+'/members/'+App.userId+'/workouts/'+today()+'/'+wid).set(entry);
  ...
  async toggleDone(wid) {
    if(!isApproved()) return;
    const w=App._room?.members?.[App.userId]?.workouts?.[today()]?.[wid]; if(!w) return;
    try { await db.ref('rooms/'+App.roomCode+'/members/'+App.userId+'/workouts/'+today()+'/'+wid+'/done').set(!w.done); }
    catch(e){ UI.toast('Error','error'); }
  },

  async delete(wid) {
    if(!isApproved()) return;
    try { await db.ref('rooms/'+App.roomCode+'/members/'+App.userId+'/workouts/'+today()+'/'+wid).remove(); UI.toast('Removed','info'); }
    catch(e){ UI.toast('Error','error'); }
  },
};
...
const Habits = {
  async toggle(key) {
    if(!isApproved()) return;
    const current=App._room?.members?.[App.userId]?.habits?.[today()]?.[key]||false;
    try { await db.ref('rooms/'+App.roomCode+'/members/'+App.userId+'/habits/'+today()+'/'+key).set(!current); }
    catch(e){ UI.toast('Error','error'); }
  },
};
...
const Weight = {
  async log() {
    ...
    const wid=genId();
    try {
      await db.ref('rooms/'+App.roomCode+'/members/'+App.userId+'/weightLog/'+wid).set({date:today(),weight:w,note,ts:Date.now()});
...
const Goals = {
  ...
  render() {
    ...
    const prog=wProg(me), weekly=wkly(me);
    const cards=[
...
let iDays=7;
const Insights = {
  setDays(n,btn){ iDays=n; document.querySelectorAll('#dayChips .chip').forEach(c=>c.classList.remove('active')); btn.classList.add('active'); this.render(); },

  render() {
    const room=App._room; if(!room) return;
    const members=o2a(room.members);
    if(!members.length){ $('insightsGrid').innerHTML='<div class="empty"><span class="empty-icon">👥</span><p>No members yet</p></div>'; $('leaderboard').innerHTML=''; return; }
    const dates=[]; for(let i=iDays-1;i>=0;i--){ const d=new Date(); d.setDate(d.getDate()-i); dates.push(d.toISOString().split('T')[0]); }
    ...
    $('insightsGrid').innerHTML=members.map(m=>{
      ...
      const streak=dates.map(d=>{ const ws=o2a(m.workouts?.[d]); if(!ws.length) return'none'; if(ws.every(w=>w.done)) return'done'; if(ws.some(w=>w.done)) return'partial'; return'none'; });
...
const Alarm = {
  save() {
    ...
    App.saveSession();
    App._syncAlarmsToSW(); // sync to service worker
  ...
  check() {
    ...
    App.alarms.forEach(a=>{ if(!a.active) return; if(a.time===cur&&a.days.includes(day)){ const k='fp_ar_'+a.id+'_'+today(); if(!sessionStorage.getItem(k)){ sessionStorage.setItem(k,'1'); this.ring(a.time); } } });
  },
...
function renderSidebar(room) {
  ...
  const mc=document.getElementById('navMemberCount'); if(mc) mc.textContent=members.length;
...
function renderTodayWorkouts(room) {
  const me=room.members?.[App.userId], wks=o2a(me?.workouts?.[today()]), el=$('workoutList');
  if(!wks.length){ el.innerHTML='<div class="empty"><span class="empty-icon">🏃</span><p>No activities yet. Hit "＋ Add" to start!</p></div>'; return; }
...
function renderWeightLog(room) {
  const me=room.members?.[App.userId], logs=o2a(me?.weightLog).sort((a,b)=>b.ts-a.ts).slice(0,10), el=$('weightList');
  if(!logs.length){ el.innerHTML='<div class="empty"><span class="empty-icon">⚖️</span><p>No weight logged yet</p></div>'; return; }
  el.innerHTML=logs.map((e,i)=>{
    const prev=logs[i+1]; let delta='';
    if(prev){ const d=e.weight-prev.weight; delta=`<span class="wl-delta ${d<0?'delta-down':'delta-up'}">${d<0?'▼':'▲'} ${Math.abs(d).toFixed(1)}kg</span>`; }
...
function wProg(m){ const logs=o2a(m.weightLog).sort((a,b)=>a.ts-b.ts),s=logs[0]?.weight??m.weight,c=logs[logs.length-1]?.weight??m.weight,d=s-(m.targetWeight||m.goals?.targetWeight||m.weight-1); if(d<=0) return 100; return Math.min(100,Math.max(0,Math.round((s-c)/d*100))); }
function latW(m){ const logs=o2a(m.weightLog).sort((a,b)=>b.ts-a.ts); return logs[0]?.weight??m.weight; }
function wkly(member){ const now=new Date(),start=new Date(now); start.setDate(start.getDate()-start.getDay()); let c=0; for(let i=0;i<7;i++){ const d=new Date(start); d.setDate(d.getDate()+i); if(o2a(member.workouts?.[d.toISOString().split('T')[0]]).some(w=>w.done)) c++; } return c; }
